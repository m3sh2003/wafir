...
const transfer_rule_entity_1 = require("./entities/transfer-rule.entity");
let RulesService = class RulesService {
    ...
    async categorizeTransaction(userId, description, amount) {
        const rules = await this.rulesRepository.find({
            where: { userId, actionType: 'CATEGORIZE', isActive: true }
        });
        for (const rule of rules) {
            ...
            if (condition && condition.contains) {
                if (description.toLowerCase().includes(condition.contains.toLowerCase())) {
                    return rule.actionValue.category;
                }
            }
        }
        return null;
...
exports.RulesService = RulesService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(transfer_rule_entity_1.TransferRule)),
